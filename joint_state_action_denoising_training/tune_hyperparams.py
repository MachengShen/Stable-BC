...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of optimization trials per environment",
    )
    parser.add_argument(
        "--envs",
        nargs="+",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Run in debug mode without try-except"
    )
    ...
    for env_name in envs_to_optimize:
        best_trial = optimize_env(
            env_name, base_config, args.n_trials, debug=args.debug
        )
        if best_trial:
            results[env_name] = {
